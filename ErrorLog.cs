...
//public static bool ErrorRoutine(bool bLogType,Exception  objException)
  public static bool ErrorRoutine(bool bLogType, string msg)
{
	try
	{
		...
		{
			if (true != CustomErrorRoutine(msg))
				return false;
		}
		return true;
	}catch(Exception)
...
{
	string strCheckinBaseDirectory	= AppDomain.CurrentDomain.BaseDirectory + "LoggingStatus.Config";

    if (File.Exists(strCheckinBaseDirectory))
        return strCheckinBaseDirectory;
	else
	{
		string strCheckinApplicationDirectory	=	GetApplicationPath() + "LoggingStatus.Config";

        if (File.Exists(strCheckinApplicationDirectory))
            return strCheckinApplicationDirectory;
		else
			return string.Empty;
	}
}
...
private static bool CustomErrorRoutine(string message)
{
    ...
    strLogFilePath = HttpContext.Current.Server.MapPath("/Debug/BHLogFile.txt");
...
private static bool WriteErrorLog(string strPathName, string msg)
{
	bool bReturn		= false;
...
private static string GetLogFilePath()
{
	try
	{
		...
		string retFilePath = baseDir + "//" + "BHLogFile.txt";
