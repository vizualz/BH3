...
namespace BoardHunt.classes.PP
{
	...
	public class wwHttp
	{
		...
		StreamReader GetUrlStream(string Url) 
		{
            ...
            if (Response == null)
            {
                ErrorLog.ErrorRoutine(false, "GetUrlStream:Response was null");
                return null;
            }

            ErrorLog.ErrorRoutine(false, "GetUrlStream:Response was OK");
		...
		{
            ErrorLog.ErrorRoutine(false, "wwHTTP:GetUrlResponse");
			...
			try 
			{
                ...
                ErrorLog.ErrorRoutine(false, "wwHTTP:Handling security");
                ...
                ErrorLog.ErrorRoutine(false, "wwHTTP:Handling proxy");
                ...
                ErrorLog.ErrorRoutine(false, "wwHTTP:Handling cookies");
                ...
                if (this.oPostData != null)
                {
                    ErrorLog.ErrorRoutine(false, "wwHTTP:oPost valid");
                }
                else
                {
                    ErrorLog.ErrorRoutine(false, "wwHTTP:oPost NULL");
                }

				// *** Deal with the POST buffer if any
				if (this.oPostData != null) 
				{
                    ErrorLog.ErrorRoutine(false, "wwHTTP:dealing with POST data");
                    ...
                    ErrorLog.ErrorRoutine(false, "wwHTTP:Getting the RequestStream");

					Stream loPostData = this.WebRequest.GetRequestStream();

                    ErrorLog.ErrorRoutine(false, "wwHTTP:Return from RequestStream");

                    if (this.SendData == null)
                    {
                        ErrorLog.ErrorRoutine(false, "wwHTTP:SendData was NULL");
                        this.oPostStream.WriteTo(loPostData);  // Simplest version - no events
                    }
                    else
                    {
                        ErrorLog.ErrorRoutine(false, "Streaming PostBuffer");
                    ...
                    ErrorLog.ErrorRoutine(false, "loPostData closed");

					// *** If user cancelled the 'upload' exit
					if (this.Cancelled) 
					{
                        ErrorLog.ErrorRoutine(false, "wwHTTP:HTTP req was cancelled");
                ...
                //PRM: not getting here
                ErrorLog.ErrorRoutine(false, "wwHTTP:Getting response");
		...
		public string GetUrl(string Url,long BufferSize) 
		{
            ...
            ErrorLog.ErrorRoutine(false, "wwHTTP:In GetURL: " + Url);
            
            StreamReader oHttpResponse = this.GetUrlStream(Url);
            if (oHttpResponse == null)
            {
                ErrorLog.ErrorRoutine(false, "HttpResponse is NULL");
                return "";
            }
            ...
            ErrorLog.ErrorRoutine(false, "lnSize: " + lnSize);
            ...
            ErrorLog.ErrorRoutine(false, "Begin to read Posted vals...");
            ...
            ErrorLog.ErrorRoutine(false, "Done reading Posted vals...");
            ...
            ErrorLog.ErrorRoutine(false, "lowriter: " + loWriter.ToString());
